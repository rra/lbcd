=for stopwords
lbcd -dhlRrt UDP DNS-based balancer lbnamed lbnamed's iptables IP
Schwimmer Allbery sublicense MERCHANTABILITY NONINFRINGEMENT
...
B<lbcd> [B<-dhlRt>] [B<-b> I<address>] [B<-c> I<command>] [B<-P> I<file>]
...
Use I<file> to store the PID of the running B<lbcd> process, rather than
the default of F</var/run/lbcd.pid>.
...
when it starts.  The file location can be changed with B<-P>.
...
2009, 2012, 2013 The Board of Trustees of the Leland Stanford Junior
University
